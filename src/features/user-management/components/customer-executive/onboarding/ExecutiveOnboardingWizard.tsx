...
import { ArrowLeft, ArrowRight, Clock, UserPlus } from "lucide-react";
...
import { ExecutiveCreateSuccessScreen } from "@/features/user-management/components/customer-executive/onboarding/ExecutiveCreateSuccessScreen";
import { ExecutiveWizardSidebar } from "@/features/user-management/components/customer-executive/onboarding/ExecutiveWizardSidebar";
import { ExecutiveAreaAssignmentStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveAreaAssignmentStep";
import { ExecutiveBasicInfoStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveBasicInfoStep";
import { ExecutiveCredentialsStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveCredentialsStep";
import { ExecutiveDocumentsStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveDocumentsStep";
import { ExecutiveOfficeDetailsStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveOfficeDetailsStep";
import { ExecutiveResponsibilitiesStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveResponsibilitiesStep";
import { ExecutiveReviewStep } from "@/features/user-management/components/customer-executive/onboarding/steps/ExecutiveReviewStep";
import {
  executiveOnboardingSchema,
  STEP_FIELD_NAMES,
  STEP_SCHEMAS,
  type ExecutiveOnboardingSchema,
...
const TOTAL_STEPS = EXECUTIVE_WIZARD_STEPS.length;
...
export function ExecutiveOnboardingWizard() {
  const draft = useExecutiveDraftStore((s) => s.draft);
  const lastSavedAt = useExecutiveDraftStore((s) => s.lastSavedAt);
  const isDirty = useExecutiveDraftStore((s) => s.isDirty);
  const setDraft = useExecutiveDraftStore((s) => s.setDraft);
  ...
  const [createdExecutive, setCreatedExecutive] =
    useState<CreateExecutiveResult | null>(null);

  const methods = useForm<ExecutiveOnboardingSchema>({
    resolver: zodResolver(executiveOnboardingSchema),
    defaultValues: draft as ExecutiveOnboardingSchema,
    mode: "onChange",
  });
  ...
  useEffect(() => {
    reset(draft as ExecutiveOnboardingSchema);
      ...
      try {
        ...
        if (!silent) {
          notify.success(
            ...
            "Your executive onboarding progress is protected.",
          );
        }
      } finally {
  ...
  const validateCurrentStep = async () => {
    ...
    if (!result.success) {
      ...
      ] as unknown as Array<keyof ExecutiveOnboardingSchema | string>;
  ...
  const renderStep = () => {
    ...
    switch (currentStep) {
      case 1:
        return <ExecutiveBasicInfoStep />;
      case 2:
        return <ExecutiveOfficeDetailsStep />;
      case 3:
        return <ExecutiveAreaAssignmentStep />;
      case 4:
        return <ExecutiveResponsibilitiesStep />;
      case 5:
        return <ExecutiveCredentialsStep />;
      case 6:
        return <ExecutiveDocumentsStep />;
      case 7:
        return <ExecutiveReviewStep onEditStep={goToStep} />;
      default:
        return null;
    }
  };
  ...
  if (createdExecutive) {
    return (
      <ExecutiveCreateSuccessScreen
        result={createdExecutive}
        onCreateAnother={handleCreateAnother}
      />
    );
  }

  const continueLabels: Record<number, string> = {
    1: "Continue to Office Details",
    2: "Continue to Area Assignment",
    3: "Continue to Responsibilities",
    4: "Continue to Login Setup",
    5: "Continue to Documents",
  ...
  return (
    <FormProvider {...methods}>
      <form
        onSubmit={handleSubmit(onCreateExecutive)}
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h1 className="text-2xl font-bold text-[#1A1A1A]">
                Create Customer Executive
              </h1>
              <p className="mt-1 text-sm text-[#64748B]">
                Complete the 7-step wizard to onboard a new customer executive
                for operational customer support.
              </p>
            </div>
            <Button
              variant="outline"
        ...
        <div className="sticky bottom-0 z-10 flex flex-col gap-3 border-t border-gray-100 bg-white/95 px-6 py-4 backdrop-blur sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            {currentStep < TOTAL_STEPS ? (
              ...
              >
                {continueLabels[currentStep] ?? "Continue"}
              ...
              >
                ...
                {isCreating ? "Creating..." : "Create Executive"}
              </Button>
            )}
          </div>
        </div>
